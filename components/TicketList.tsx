...
export const TicketList: React.FC<TicketListProps> = ({ currentUser }) => {
  ...
  useEffect(() => {
    const getData = async () => {
      apiRequest<Ticket[]>("/tickets", "GET", { authToken: token }).then(
        (res) => {
          setTickets(res);
        },
      );
  ...
  const assignedActive = sortTickets(
    tickets?.filter(
      (t) =>
  ...
  const assignedHistory = sortTickets(
    tickets?.filter(
      (t) =>
        t.assigneeId === currentUser.id &&
        (t.status === TicketStatus.RESOLVED ||
  ...
  const renderTable = (
    ...
    <TableContainer
      component={Paper}
      className="shadow-sm border border-gray-200 rounded-xl overflow-hidden mb-8"
    >
      <Table sx={{ minWidth: 650 }} aria-label="ticket table">
        <TableHead className="bg-gray-50">
          <TableRow>
            ...
            <TableCell className="font-bold text-[#e51b24]">
              Fecha Entrega
            </TableCell>
            <TableCell className="font-bold text-gray-600" align="center">
              Tiempo Transcurrido
            </TableCell>
            <TableCell align="right" className="font-bold text-gray-600">
              Acción
            </TableCell>
          </TableRow>
        </TableHead>
          ...
          {tickets?.length > 0 ? (
            tickets?.map((ticket) => {
              ...
              return (
                ...
                >
                  <TableCell className="font-medium text-[#1e242b]">
                    {ticket.id}
                  </TableCell>
                  ...
                  <TableCell className="font-medium">
                    <div className="flex items-center gap-2">
                      {ticket?.subject}
                      {isOverdue && (
                        <AlertCircle size={14} className="text-[#e51b24]" />
                      )}
                    </div>
                  </TableCell>
                  ...
                  <TableCell>
                    <span
                      className={`font-medium ${
                        isOverdue ? "text-[#e51b24] font-bold" : "text-gray-600"
                      }`}
                    >
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-12">
      ...
      <div>
        ...
        {renderTable(
          assignedHistory,
          "No tienes tickets finalizados recientemente.",
          false,
        )}
      </div>
